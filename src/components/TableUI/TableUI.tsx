...
// 	// Reset pageIndex when filters/search change
// 	// Removed columnKeyMap. Now using config.columns directly for filtering.

// 	// Filter columns based on config (filters logic commented out)
// 	const columns = useMemo(() => {
// 		if (!config?.columns) return allColumns;
// 		return allColumns.filter(col => {
// 			// For select column
// 			if (col.id === 'select') return false;
// 			// For others, match by header or accessorKey directly
// 			return config.columns.includes(col.header) || config.columns.includes(col.accessorKey);
// 		});
// 	}, [config, allColumns]);

// 	// Filtering logic is commented out as per instructions
// 	// ...existing code...
// 	useEffect(() => {
// 		setPageIndex(0);
// 	}, [assignedTo, status, submited, search]);
